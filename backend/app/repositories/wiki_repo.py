...
_WIKI_SCHEMA = """\
# Wiki Schema — MemRAG Knowledge Base
...
│   ├── documents/          # Văn bản trích xuất từ PDF/Word/Excel
│   └── transcripts/        # Transcript hội thoại
├── pages/                  # TẦNG 2: TRI THỨC ĐÃ BIÊN DỊCH
│   ├── entities/           # Models, frameworks, datasets, tools, people, orgs, methods, concepts
│   ├── topics/             # Chủ đề cấp cao — vấn đề, hướng, lĩnh vực
...
### entities/
Trang về thực thể CÓ THỂ ĐỊNH DANH RÕ RÀNG — phủ rộng mọi lĩnh vực:
- **model**: AI/ML model, statistical model, mathematical model (GPT-4o, ResNet, BERT, ARIMA)
- **framework**: thư viện, platform, standard (PyTorch, React, Kubernetes, REST, GraphQL)
- **dataset**: tập dữ liệu (ImageNet, MMLU, World Bank, Kaggle datasets)
- **benchmark**: bộ đánh giá, metric system (HELM, SWE-bench, LeetCode, SPEC, TPC-C)
- **researcher**: tác giả, chuyên gia (Yann LeCun, Andrew Ng, John Carmack)
- **lab**: tổ chức, công ty (OpenAI, Google, MIT, Stanford, NASA, WHO)
- **tool**: công cụ, phần mềm (Docker, PostgreSQL, Redis, Git, VS Code, Qdrant)
- **method**: algorithm/quy trình CỤ THỂ có thể implement — bất kỳ lĩnh vực (ReAct, LoRA, CI/CD, Scrum, Monte Carlo, MapReduce)
- **concept**: kiến trúc/paradigm TỔNG QUÁT (Transformer, Microservices, REST, ACID, Blockchain, Serverless, Edge Computing)

### topics/
Trang về CHỦ ĐỀ cấp độ cao — vấn đề/hướng quan trọng:
- Bất kỳ lĩnh vực: công nghệ, kinh doanh, khoa học, giáo dục, y tế, môi trường...
- Ví dụ: "Cloud Migration Strategy", "Data Privacy Compliance", "Performance Optimization", "System Reliability"

### summaries/
Tóm tắt chuyên sâu từng nguồn:
- Mỗi document/meeting có đúng **1 summary page**
- Format: TL;DR → Điểm chính → Chi tiết → Kết luận → Hạn chế

### Benchmarks & Mathematical Foundations (trong entities/)
- Benchmark pages: bảng so sánh kết quả nhiều models/systems + trend analysis
- Mathematical concept pages: công thức LaTeX $$...$$ + giải thích biến số
...
- **Link nội bộ**: `[[pages/entities/slug.md]]`, `[[pages/topics/slug.md]]`, `[[pages/summaries/slug.md]]`
  - Slug chỉ gồm `[a-z0-9]`: "U-Net" → `[[pages/entities/unet.md]]`, "CI/CD" → `[[pages/entities/cicd.md]]`
- **Trích dẫn nguồn**: `[tên nguồn]` (dấu ngoặc đơn)
- Giữ thuật ngữ chuyên ngành bằng gốc
- Mâu thuẫn: `~~thông tin cũ [nguồn cũ]~~` → thông tin mới [nguồn mới]
- Khi có mâu thuẫn: thêm section `## Lịch sử / Tiến triển` dạng bảng

## Nguyên tắc cập nhật

1. **Entities**: trích xuất TẤT CẢ — không bỏ sót
2. **Topics**: chọn tối đa 3 chủ đề quan trọng nhất của nguồn
3. **Summary**: luôn tạo/cập nhật 1 trang tóm tắt cho mỗi nguồn
4. **Không xóa lịch sử**: strikethrough thay vì xóa
